...
export const ZapApiRequestEditor: React.FC = () => {
  ...
  const isZapView = state.viewState.activeView === 'zap-editor';

  if (isZapView) {
    // .zap file editor view
    const zapContent = `meta {
...
${activeRequest.body.content}
    ...
    return (
      <div className="void-h-full void-bg-[var(--vscode-editor-background)] void-text-[var(--vscode-editor-foreground)]">
				{/* Header */}
            ...
            onClick={() => actions.toggleCenterView()}>
				...
				<div className="void-p-4 void-h-full void-overflow-auto">
					<pre className="void-text-sm void-whitespace-pre-wrap void-font-mono void-bg-[var(--vscode-textCodeBlock-background)] void-p-4 void-rounded">
						{zapContent}
					</pre>
				</div>
			</div>);
  ...
  return (
    <div className="void-h-full void-bg-[var(--vscode-editor-background)] void-text-[var(--vscode-editor-foreground)] void-flex void-flex-col">
            ...
            readOnly />
					...
					<button
            className="void-px-4 void-py-1 void-text-sm void-bg-[var(--vscode-button-background)] void-text-[var(--vscode-button-foreground)] void-rounded hover:void-bg-[var(--vscode-button-hoverBackground)]"
            onClick={() => actions.sendRequest(activeRequest)}>
				...
				<button
          className="void-px-2 void-py-1 void-text-xs void-bg-[var(--vscode-button-secondaryBackground)] void-text-[var(--vscode-button-secondaryForeground)] void-rounded hover:void-bg-[var(--vscode-button-secondaryHoverBackground)]"
          onClick={() => actions.toggleCenterView()}
          title="View raw .bru file">
			...
			<div className="void-flex-1 void-flex">
				...
				<div className="void-w-1/2 void-border-r void-border-[var(--vscode-panel-border)] void-flex void-flex-col">
					...
					<div className="void-flex-1 void-p-3">
						{activeRequest.body?.content ?
            <pre className="void-text-sm void-whitespace-pre-wrap void-font-mono void-bg-[var(--vscode-textCodeBlock-background)] void-p-3 void-rounded">
								{activeRequest.body.content}
							</pre> :

            <div className="void-text-[var(--vscode-descriptionForeground)] void-text-sm">No request body</div>
            }
					</div>
				</div>
            ...
            <div className="void-text-[var(--vscode-descriptionForeground)] void-text-sm">Click "Send" to see response</div>
            }
					</div>
				</div>
			</div>
